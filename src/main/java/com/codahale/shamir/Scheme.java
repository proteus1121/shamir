...
public class Scheme {
  ...
  public Map<Integer, Pair<byte[], byte[]>> splitByFeldman(byte[] secret) {
    // generate part values
    final byte[][] values = new byte[n][secret.length];
    ...
    // return as a set of objects
    final Map<Integer, Pair<byte[], byte[]>> parts = new HashMap<>(n());
    for (int i = 0; i < values.length; i++) {
      parts.put(i + 1, new Pair<>(values[i], checkSum.get(i)));
    }
    return Collections.unmodifiableMap(parts);
  }
