...
public class TestFeldman {
  ...
  public void testFeldman() {
    ...
    Map<Integer, Pair<byte[], byte[]>> integerPairMap = scheme.splitByFeldman(secret);
    ArrayList<Long> collect =
        (ArrayList<Long>)
            integerPairMap
                ...
                .stream()
                .map(Pair::getValue)
                .map(ByteUtils::bytesToLong)
                .collect(Collectors.toList());
    FeldmanVSS F = new FeldmanVSS(5, 3);
    F.setCommitments(msg, collect);
    String result = F.verifyShare(0, ByteUtils.bytesToLong(integerPairMap.get(0).getKey()));
